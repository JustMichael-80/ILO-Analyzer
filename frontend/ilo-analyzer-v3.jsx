...
const VERDICT_STYLE = (verdict = "") => {
  ...
  if (lower.includes("clean")) return { border: "border-emerald-500/40", bg: "bg-emerald-950/20", text: "text-emerald-400", bar: "#34d399" };
  if (lower.includes("suppression")) return { border: "border-amber-500/40", bg: "bg-amber-950/20", text: "text-amber-400", bar: "#fbbf24" };
  if (lower.includes("anomalous")) return { border: "border-cyan-500/40", bg: "bg-cyan-950/20", text: "text-cyan-400", bar: "#22d3ee" };
  ...
  const simulatePhases = () => {
    ...
    phaseTimer.current = setInterval(() => {
      ...
      if (idx < PHASES.length) {
        ...
        setCurrentPhase("Passing matrices to P4 Gate for diagnostic validation...");
      }
    }, 1800);
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-mono p-4 md:p-6 selection:bg-teal-500 selection:text-slate-950">
        ...
        <div className="flex justify-between items-start flex-wrap gap-3">
          <div>
            ...
            <p className="text-[11px] text-slate-500 mt-1">
              Sieve Module v2.1.0 // Powered by the Principle of Persistent Structurization
            </p>
          </div>
          ...
          {verdict ? (
            <div className="space-y-4">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded p-5 space-y-4">
                <div className="px-1"><PiBar value={verdict.pi_final || 0} /></div>
      ...
      <footer className="max-w-5xl mx-auto mt-8 pt-4 border-t border-slate-900 flex justify-between items-center flex-wrap gap-2">
        <span className="text-[9px] text-slate-700 uppercase tracking-widest">
          ChronoDyne Systems · Principle of Persistent Structurization · ILO Framework v2.1.0
        </span>
        <span className="text-[9px] text-slate-700 uppercase tracking-widest">
          P4 Gate · FastAPI Engine Gateway
        </span>
      </footer>
    </div>
  );
